#word2vec
#介绍了两种模型，这两种模型可以一定程度上优化one-hot词向量无法表示不同词相似度的问题
#这里实现的是skip-gram跳字模型，假设背景词是由中心词生成的
import collections
import math
import random
import sys
...
#预处理-建立双向索引
counter = collections.Counter([tk for st in raw_dataset for tk in st]) #对出现于读取到数据的每个句子中的每个词进行词频统计
...
#预处理-二次采样
def discard(idx):#概率性丢弃一些高频词，例如a，the，in等词有时并无实际意义，在超过丢弃阈值后词频越高丢弃概率越高
    '''
    @params:
        idx: 单词的下标
    ...
    return random.uniform(0, 1) < 1 - math.sqrt(
        1e-4 / counter[idx_to_token[idx]] * num_tokens)#1e-4是一个超参数
...
窗口长度  2
中心词    1 2 3 4 5
背景词    [2,3] [1,3,4] [1,2,4,5] [2,3,5] [3,4]
'''
...
#负采样
def get_negatives(all_contexts, sampling_weights, K):#采用该方法解决词典中任意词都是中心词从而导致计算开销过大的问题
    '''
    @params:
...
#损失函数-二元交叉损失熵-由负采样的最大似然估计的对数等价形式得到
